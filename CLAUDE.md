...
3. `main()` — CLI argument parsing, writes the output file. Exits with code 1 on any error (bad args, parse failure, write failure).

**Regex patterns** are compiled once as class-level `_RE_*` constants on both `Track` and `Cuesheet`, not on every `parseline()` call.

**`track_complete()`** requires `track_num`, `title`, and `index` to all be set. A track with a per-track PERFORMER but no TITLE is considered incomplete and is not added to the sheet.
